...
if __name__ == "__main__":
    ...
    video_1 = "videos/192-168-100-22/12/"
    video_2 = "videos/192-168-100-32/12/"

    gate_zone_1 = BOS_CAM1_ZONE
    gate_zone_2 = BOS_CAM2_ZONE

    # Step 1: Create a json file to store the exit timestamps
    exit_timestamps_12_1 = {}
    exit_timestamps_12_2 = {}
    ...
    for video in os.listdir(video_1):
        ...
        if video.endswith('.mp4'):
            exit_timestamps_12_1[video] = detect_motion_timestamps(video_path, gate_zone_1)
        else:
            continue
    ...
    for video in os.listdir(video_2):
        ...
        # Check if video is a mp4 file
        if video.endswith('.mp4'):
            exit_timestamps_12_2[video] = detect_motion_timestamps(video_path)
        else:
            continue

    # Step 3: Save the exit timestamps to the json file
    with open('exit_timestamps_12_1.json', 'w') as f:
        json.dump(exit_timestamps_12_1, f)
    with open('exit_timestamps_12_2.json', 'w') as f:
        json.dump(exit_timestamps_12_2, f)
